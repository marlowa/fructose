...
class XunitFileParser(TestFileParser):
    ...
    """
    CLASS_RE = re.compile(r'\b(?:struct|class)\s+([a-zA-Z_0-9]\w*)\s+:\s+public\s+(?:fructose\s*::\s*)?test_base\s*<\s*\1\s*>')
    CLASS_MACRO_RE = re.compile(r'\bFRUCTOSE_(?:CLASS|STRUCT)\s*\(([a-zA-Z0-9_]*)')
    CASE_RE = re.compile(r'\bvoid\s+(test\w+)\s*\(const\s+(?:std::)?string\s*&(?:\s+[a-zA-Z_]\w+)?\s*\)')
...
def main(argv=None):
    ...
    parser = optparse.OptionParser(usage=USAGE, description=DESCRIPTION,
                                   version=VERSION)
    parser.set_defaults(generator=False)
    parser.add_option("-g", "--generator", action="store_true", help="use generator style code generation [default: %default]")
